...
namespace Demo1
{
	class Program
	{
		...
		static bool loadDemoData(NVGcontext vg, DemoData data)
		{
			for (int i = 0; i < 12; i++)
			{
		...
		static void drawButton(NVGcontext vg, int preicon, string text, float x, float y, float w, float h, NVGcolor col)
		{
			NVGpaint bg;
		...
		static void drawWindow(NVGcontext vg, string title, float x, float y, float w, float h)
		{
		...
		static void drawSearchBox(NVGcontext vg, string text, float x, float y, float w, float h)
		{
			NVGpaint bg;
			//char icon[8];
		...
		static void drawDropDown(NVGcontext vg, string text, float x, float y, float w, float h)
		{
			NVGpaint bg;
			//char icon[8];
			float cornerRadius = 4.0f;
		...
		static void drawLabel(NVGcontext vg, string text, float x, float y, float w, float h)
		{
			//NVG_NOTUSED(w);
		...
		static void drawEditBox(NVGcontext vg, string text, float x, float y, float w, float h)
		{
		...
		static void drawEditBoxNum(NVGcontext vg, string text, string units, float x, float y, float w, float h)
		{
			float uw;
		...
		static void drawCheckBox(NVGcontext vg, string text, float x, float y, float w, float h)
		{
			NVGpaint bg;
			//char icon[8];
			//NVG_NOTUSED(w);
		...
		static void drawSlider(NVGcontext vg, float pos, float x, float y, float w, float h)
		{
			NVGpaint bg, knob;
		...
		static void drawWidths(NVGcontext vg, float x, float y, float width)
		{
			int i;
		...
		static void drawCaps(NVGcontext vg, float x, float y, float width)
		{
			int i;
			int[] caps = new int[]
			{ 
		...
		static void drawScissor(NVGcontext vg, float x, float y, float t)
		{
			NanoVG.nvgSave(vg);
		...
		static void renderDemo(NVGcontext vg, float mx, float my, float width, float height,
		...
		public static void Main(string[] args)
		{
			...
			try
			{
				...
				try
				{
					...
					// Create GLFW window
					GlfwWindowPtr window = Glfw.CreateWindow(width, height, "NanoVG.net-Pencil.Gaming.GLFW3", GlfwMonitorPtr.Null, GlfwWindowPtr.Null);
					...
					while (!Glfw.WindowShouldClose(window))
					{
						...
						dt = t - prevt;
						prevt = t;
						PerfGraph.UpdateGraph((float)dt);
						...
						NanoVG.nvgBeginFrame(vg, winWidth, winHeight, pxRatio);

						renderDemo(vg, (float)mx, (float)my, winWidth, winHeight, (float)t, blowup, data);
